...
class DB {
  ...
  async getUserByEmail(email) {
    ...
    try {
      ...
      return { ...user, roles: roles, password: undefined }; // Exclude password
    } finally {
      connection.end();
    }
  }
  ...
  async loginUser(userId, token) {
    ...
    try {
      ...
      // Use REPLACE INTO to avoid duplicate entries
      await this.query(
        connection,
        `REPLACE INTO auth (userId, token) VALUES (?, ?)`,
        [userId, token]
      );
    } finally {
      connection.end();
  ...
  async isLoggedIn(token) {
    ...
    try {
      ...
      return authResult.length > 0; // Returns true if token is valid
    } finally {
      connection.end();
    }
  }
